<template>
  <div class="page">
    ...
    <van-popup v-model="this.$store.state.showadvertising" :close-on-click-overlay="false" class="popup-box">
      <van-row>
        <van-col :offset=21 :span=2 style="margin-bottom: 0.2rem">
          <svg class="icon icon-large" aria-hidden="true" @click="closeAd()">
            <use xlink:href="#icon-close"></use>
          </svg>
        </van-col>
        <van-col :offset=4 :span=16>
          <img src="../../static/img/goods/home_floor_five_1.jpg" alt="advertising" width="100%" />
        </van-col>
        <van-col :offset=4 :span=16 style="margin-top: 0.5rem;">
          <van-progress :percentage="percent" color="#f3594b" pivot-text="Closeing" />
        </van-col>
      </van-row>
    </van-popup>
    ...
    onAddCart(item, name) {
      if (!this.carts.includes(item)) {
        if (!this.$refs.footer.showNum) {
          this.cartLength = this.carts.length + 1
